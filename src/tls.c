...
static void TLSX_KeyShare_FreeAll(KeyShareEntry* list, void* heap)
{
    ...
    while ((current = list) != NULL) {
        ...
        if (WOLFSSL_NAMED_GROUP_IS_FFDHE(current->group)) {
            ...
            wc_KyberKey_Free((KyberKey*)current->hybridKey);
            XFREE(current->hybridKey, heap, DYNAMIC_TYPE_PRIVATE_KEY);
                                             ...
                                             word32* ssOutSz)
{
    ...
    if (keyShareEntry->ke == NULL || kemKey == NULL) {
                                                ...
                                                KeyShareEntry* keyShareEntry)
{
    ...
    if (ret == 0) {
        XMEMSET(pqc_kse, 0, sizeof(*pqc_kse));
        pqc_kse->group = pqc_group;
        pqc_kse->key = keyShareEntry->hybridKey;
    ...
    if (ret == 0) {
        keyShareEntry->hybridKey = pqc_kse->key;
