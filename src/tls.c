...
static int TLSX_Cert_With_Extern_Psk_Parse(WOLFSSL* ssl, byte msgType)
{
    int ret = 0;

    /* When we parse a ClientHello on the server side, the certWithExternPsk
     * must be explicity set to use the extension further on.
     * On the client side, we directy use it. This is necessary, as we
     * explicitly disable the option after sending it to the server to only
     * re-enable it if the server answers with the extension.  */
    if ((msgType == client_hello && ssl->options.certWithExternPsk == 1) ||
    ...
    while ((extension = list)) {
        ...
        switch (extension->type) {
            ...
            case TLSX_CERT_WITH_EXTERN_PSK:
                WOLFSSL_MSG("Cert with extern PSK extension free");
                break;
        #endif
        #endif
    #endif
...
int TLSX_PopulateExtensions(WOLFSSL* ssl, byte isServer)
{
    ...
    ret = 0;
                ...
                if (ssl->arrays->psk_keySz > 0 && identitySz > 0) {
                    ...
                    for (i = 0; i < suites->suiteSz; i += 2) {
                        ...
                        ret = TLSX_PreSharedKey_CreateImportedIdentity(
                                (byte*)ssl->arrays->client_identity, identitySz,
                                ctx, ctxSz,
                                SuiteMac(WOLFSSL_SUITES(ssl)->suites + i),
                                ssl->version, NULL, &importedIdentitySz);
                        if (ret != 0) {
                        #ifdef WOLFSSL_SMALL_STACK
                            XFREE(ctx, ssl->heap, DYNAMIC_TYPE_TMP_BUFFER);
                        #endif
                        ...
                        ret = TLSX_PreSharedKey_CreateImportedIdentity(
                                ...
                                ctx, ctxSz,
                                SuiteMac(WOLFSSL_SUITES(ssl)->suites + i),
                                ssl->version, importedIdentity,
                                &importedIdentitySz);
                        if (ret != 0) {
                        #ifdef WOLFSSL_SMALL_STACK
                            XFREE(ctx, ssl->heap, DYNAMIC_TYPE_TMP_BUFFER);
                        #endif
                            XFREE(importedIdentity, ssl->heap,
                                  DYNAMIC_TYPE_TLSX);
                            return ret;
                        }
                        ...
                        ret = TLSX_PreSharedKey_Use(&ssl->extensions,
                                ...
                                cipherSuite0, cipherSuite, 0, &psk, ssl->heap);
                        if (ret != 0) {
                        #ifdef WOLFSSL_SMALL_STACK
                            XFREE(ctx, ssl->heap, DYNAMIC_TYPE_TMP_BUFFER);
                        #endif
        ...
        if((usingPSK) && (ssl->options.certWithExternPsk)) {
            ret = TLSX_Cert_With_Extern_Psk_Use(ssl);
            /* Disable the option here to make sure it is only used in case
             * the server also sends the extension. */
            ssl->options.certWithExternPsk = 0;
            if (ret != 0)
                return ret;
        }
