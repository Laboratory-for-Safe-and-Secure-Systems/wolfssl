...
int DeriveImportedPreSharedKey(WOLFSSL* ssl, PreSharedKey* psk)
{
    ...
    if (ret == 0) {
        ...
        if (ret == 0) {
            ret = wc_Hkdf(hkdf, ssl->arrays->psk_key, ssl->arrays->psk_keySz,
                          NULL, 0, hkdfLabel, idx, ssl->arrays->psk_key,
                          outputLen);
            wc_HkdfFree(hkdf);
        }
        PRIVATE_KEY_LOCK();
        ...
        word32* psk_keySz, const byte* suite, int* found, byte* foundSuite)
{
    ...
    *found = 0;
    ...
    if (ssl->options.server_psk_importer_cb != NULL) {
        ...
        byte targetKdf = 0;
        unsigned int extPskSz = MAX_PSK_KEY_LEN;
        ...
        if (TLSX_PreSharedKey_ParseImportedIdentity(psk->identity,
            ...
            if (SuiteMac(suite) == targetKdf) {
                ...
                ret = ssl->options.server_psk_importer_cb((WOLFSSL*)ssl,
                    (const char*)id, ctx, ctxSz, psk_key, &extPskSz);

                if (ret == 0 && extPskSz > 0 && extPskSz <= MAX_PSK_KEY_LEN) {
