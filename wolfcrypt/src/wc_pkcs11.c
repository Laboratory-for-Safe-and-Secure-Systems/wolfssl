                                      ...
                                      CK_MECHANISM_INFO_PTR mechInfo)
{
    ...
    CK_ATTRIBUTE    keyTemplate[] = {
        ...
        { CKA_VALUE,         NULL,          0                    },
    #ifdef WOLFSSL_PKCS11_EVIDEN_FIX
    ...
    if (ret == 0) {
        ...
        ret = wc_MlKemKey_EncodePublicKey(key, publicKey, publicKeyLen);
    }
...
#endif
#endif /* HAVE_PQC && WOLFSSL_HAVE_MLKEM */
    ...
    if (ret == 0) {
        ...
        *count = 0;
        ...
        do {
            ...
            PKCS11_RV("C_FindObjects", rv);
            PKCS11_VAL("C_FindObjects Count", found);
            if (rv != CKR_OK) {
                ret = WC_HW_E;
                break;
            }
...
#endif /* !NO_PKCS11_ECDH || (HAVE_PQC && WOLFSSL_HAVE_MLKEM) */
...
static int Pkcs11EcKeyGen(Pkcs11Session* session, wc_CryptoInfo* info)
{
    ...
    /* Mandatory entries + 3 optional. */
    int              pubTmplCnt = sizeof(pubKeyTmpl) / sizeof(*pubKeyTmpl) - 3;
    ...
    /* Mandatory entries + 3 optional. */
    int              privTmplCnt = sizeof(privKeyTmpl) / sizeof(*privKeyTmpl) - 3;
...
#if defined(HAVE_PQC) && defined(WOLFSSL_HAVE_MLKEM)
/**
 * Find the PKCS#11 object containing the ML-KEM public or private key data.
 *
...
/**
 * Gets the public key data from the PKCS#11 object and puts into
 * the ML-KEM key.
 *
 * @param  [in]  key       ML-KEM key.
 * @param  [in]  session   Session object.
 * @param  [in]  keyHandle ML-KEM public key PKCS#11 object.
 * @return  WC_HW_E when a PKCS#11 library call fails.
 * @return  MEMORY_E when a memory allocation fails.
 * @return  0 on success.
 */
static int Pkcs11GetMlKemPublicKey(MlKemKey* key,
                                   Pkcs11Session* session,
                                   CK_OBJECT_HANDLE keyHandle)
{
    ...
    if (ret == 0)
        ret = wc_MlKemKey_DecodePublicKey(key, pubKey, pubKeySize);
...
static int Pkcs11PqcKemEncapsulate(Pkcs11Session* session, wc_CryptoInfo* info)
{
    ...
    if (ret == 0) {
        WOLFSSL_MSG("PKCS#11: PQC ML-KEM Encapsulation Operation");

        ret = Pkcs11CreateMlKemPublicKey(&publicKey, session,
...
static int Pkcs11PqcKemDecapsulate(Pkcs11Session* session, wc_CryptoInfo* info)
{
    ...
    if (ret == 0) {
        WOLFSSL_MSG("PKCS#11: PQC ML-KEM Decapsulate Operation");
...
static int Pkcs11Hkdf(Pkcs11Session* session, wc_CryptoInfo* info)
{
    ...
    /* Empty attribute for VALUE_LEN in case of expand operation */
    CK_ATTRIBUTE      tmpl[] = {
        { CKA_CLASS,       &secretKeyClass, sizeof(secretKeyClass) },
    ...
    if (ret == 0) {
        ...
        if ((hkdf->idLen == 0) && (hkdf->labelLen == 0) &&
            ...
            sessionKey = 1;
            ret = Pkcs11CreateSecretKey(&baseKey, session, CKK_HKDF,
                                        (byte*)hkdf->key, hkdf->keyLen, NULL,
                                        0, NULL, 0, CKA_DERIVE, 0);
            if (ret == WC_NO_ERR_TRACE(WC_HW_E)) {
                ret = Pkcs11CreateSecretKey(&baseKey, session,
                                        CKK_GENERIC_SECRET, (byte*)hkdf->key,
                                        hkdf->keyLen, NULL, 0, NULL, 0,
                                        CKA_DERIVE, 0);
            }
        }
    ...
    #ifdef WOLFSSL_PKCS11_EVIDEN_FIX
            ret = Pkcs11FindKeyByLabel(&baseKey, CKO_SECRET_KEY, inKeyType,
                                       session, hkdf->label, hkdf->labelLen);
    #else
            ret = Pkcs11FindKeyByLabel(&baseKey, CKO_SECRET_KEY, CKK_HKDF,
                                       session, hkdf->label, hkdf->labelLen);
            if (ret == WC_NO_ERR_TRACE(WC_HW_E)) {
                ret = Pkcs11FindKeyByLabel(&baseKey, CKO_SECRET_KEY,
                                           CKK_GENERIC_SECRET, session,
                                           hkdf->label, hkdf->labelLen);
            }
    #endif
        }
        else if (hkdf->idLen != 0) {
    #ifdef WOLFSSL_PKCS11_EVIDEN_FIX
            ret = Pkcs11FindKeyById(&baseKey, CKO_SECRET_KEY, inKeyType,
                                    session, hkdf->id, hkdf->idLen);
    #else
            ret = Pkcs11FindKeyById(&baseKey, CKO_SECRET_KEY, CKK_HKDF,
                                    session, hkdf->id, hkdf->idLen);
            if (ret == WC_NO_ERR_TRACE(WC_HW_E)) {
                ret = Pkcs11FindKeyById(&baseKey, CKO_SECRET_KEY,
                                        CKK_GENERIC_SECRET, session,
                                        hkdf->id, hkdf->idLen);
            }
    #endif
    ...
    if (ret == 0 && info->hkdf.expand) {
        derivedKeyLen = info->hkdf.outSz;
        tmpl[tmplCnt].type = CKA_VALUE_LEN;
        tmpl[tmplCnt].pValue = &derivedKeyLen;
        tmpl[tmplCnt].ulValueLen = sizeof(derivedKeyLen);
        tmplCnt++;
    }
