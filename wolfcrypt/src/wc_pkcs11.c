                                      ...
                                      CK_MECHANISM_INFO_PTR mechInfo)
{
    ...
    if ((key->type == KYBER512) &&
            (mechInfo->ulMinKeySize <= 512) &&
            (mechInfo->ulMaxKeySize >= 512)) {
        publicKeyLen = KYBER512_PUBLIC_KEY_SIZE;
        param_set = CKP_ML_KEM_512;
    }
    else if ((key->type == KYBER768) &&
             (mechInfo->ulMinKeySize <= 768) &&
             (mechInfo->ulMaxKeySize >= 768)) {
        publicKeyLen = KYBER768_PUBLIC_KEY_SIZE;
        param_set = CKP_ML_KEM_768;
    }
    else if ((key->type == KYBER1024) &&
             (mechInfo->ulMinKeySize <= 1024) &&
             (mechInfo->ulMaxKeySize >= 1024)) {
        publicKeyLen = KYBER1024_PUBLIC_KEY_SIZE;
                                       ...
                                       CK_MECHANISM_INFO_PTR mechInfo)
{
    ...
    if ((key->type == KYBER512) &&
            (mechInfo->ulMinKeySize <= 512) &&
            (mechInfo->ulMaxKeySize >= 512)) {
        privateKeyLen = KYBER512_PRIVATE_KEY_SIZE;
        param_set = CKP_ML_KEM_512;
    }
    else if ((key->type == KYBER768) &&
             (mechInfo->ulMinKeySize <= 768) &&
             (mechInfo->ulMaxKeySize >= 768)) {
        privateKeyLen = KYBER768_PRIVATE_KEY_SIZE;
        param_set = CKP_ML_KEM_768;
    }
    else if ((key->type == KYBER1024) &&
             (mechInfo->ulMinKeySize <= 1024) &&
             (mechInfo->ulMaxKeySize >= 1024)) {
        privateKeyLen = KYBER1024_PRIVATE_KEY_SIZE;
        param_set = CKP_ML_KEM_1024;
    }
    else {
        ret = NOT_COMPILED_IN;
                                          ...
                                          int persistent)
{
    ...
    if ((key->level == 2) &&
            (mechInfo->ulMinKeySize <= DILITHIUM_LEVEL2_KEY_SIZE) &&
            (mechInfo->ulMaxKeySize >= DILITHIUM_LEVEL2_KEY_SIZE)) {
        publicKeyLen = DILITHIUM_LEVEL2_PUB_KEY_SIZE;
        param_set = CKP_ML_DSA_44;
    }
    else if ((key->level == 3) &&
             (mechInfo->ulMinKeySize <= DILITHIUM_LEVEL3_KEY_SIZE) &&
             (mechInfo->ulMaxKeySize >= DILITHIUM_LEVEL3_KEY_SIZE)) {
        publicKeyLen = DILITHIUM_LEVEL3_PUB_KEY_SIZE;
        param_set = CKP_ML_DSA_65;
    }
    else if ((key->level == 5) &&
             (mechInfo->ulMinKeySize <= DILITHIUM_LEVEL5_KEY_SIZE) &&
             (mechInfo->ulMaxKeySize >= DILITHIUM_LEVEL5_KEY_SIZE)) {
        publicKeyLen = DILITHIUM_LEVEL5_PUB_KEY_SIZE;
                                           ...
                                           int persistent)
{
    ...
    if ((key->level == 2) &&
            (mechInfo->ulMinKeySize <= DILITHIUM_LEVEL2_KEY_SIZE) &&
            (mechInfo->ulMaxKeySize >= DILITHIUM_LEVEL2_KEY_SIZE)) {
        privateKeyLen = DILITHIUM_LEVEL2_KEY_SIZE;
        param_set = CKP_ML_DSA_44;
    }
    else if ((key->level == 3) &&
             (mechInfo->ulMinKeySize <= DILITHIUM_LEVEL3_KEY_SIZE) &&
             (mechInfo->ulMaxKeySize >= DILITHIUM_LEVEL3_KEY_SIZE)) {
        privateKeyLen = DILITHIUM_LEVEL3_KEY_SIZE;
        param_set = CKP_ML_DSA_65;
    }
    else if ((key->level == 5) &&
             (mechInfo->ulMinKeySize <= DILITHIUM_LEVEL5_KEY_SIZE) &&
             (mechInfo->ulMaxKeySize >= DILITHIUM_LEVEL5_KEY_SIZE)) {
        privateKeyLen = DILITHIUM_LEVEL5_KEY_SIZE;
        param_set = CKP_ML_DSA_87;
    }
    else {
        ret = NOT_COMPILED_IN;
                                   ...
                                   CK_OBJECT_HANDLE keyHandle)
{
    ...
    if (ret == 0) {
        if (pubKeySize == KYBER512_PUBLIC_KEY_SIZE)
            key->type = KYBER512;
        else if (pubKeySize == KYBER768_PUBLIC_KEY_SIZE)
            key->type = KYBER768;
        else if (pubKeySize == KYBER1024_PUBLIC_KEY_SIZE)
            key->type = KYBER1024;
        else
            ret = WC_KEY_SIZE_E;
    }
...
static int Pkcs11PqcKemKeyGen(Pkcs11Session* session, wc_CryptoInfo* info)
{
    ...
    if (ret == 0) {
        if (key->type == KYBER512) {
            if (mechInfo.ulMinKeySize <= 512 && mechInfo.ulMaxKeySize >= 512)
                    paramSet = CKP_ML_KEM_512;
            else
                ret = NOT_COMPILED_IN;
        }
        else if (key->type == KYBER768) {
            if (mechInfo.ulMinKeySize <= 768 && mechInfo.ulMaxKeySize >= 768)
                    paramSet = CKP_ML_KEM_768;
            else
                ret = NOT_COMPILED_IN;
        }
        else if (key->type == KYBER1024) {
            if (mechInfo.ulMinKeySize <= 1024 && mechInfo.ulMaxKeySize >= 1024)
...
static int Pkcs11PqcKemEncapsulate(Pkcs11Session* session, wc_CryptoInfo* info)
{
    ...
    if (ret == 0)
    {
        if ((mechInfo.flags & CKF_ENCAPSULATE) == 0)
            ret = NOT_COMPILED_IN;
        else if ((key->type == KYBER512) &&
                 (mechInfo.ulMinKeySize > 512 || mechInfo.ulMaxKeySize < 512))
            ret = NOT_COMPILED_IN;
        else if ((key->type == KYBER768) &&
                 (mechInfo.ulMinKeySize > 768 || mechInfo.ulMaxKeySize < 768))
            ret = NOT_COMPILED_IN;
        else if ((key->type == KYBER1024) &&
                 (mechInfo.ulMinKeySize > 1024 || mechInfo.ulMaxKeySize < 1024))
            ret = NOT_COMPILED_IN;
    }

    if (ret == 0) {
        WOLFSSL_MSG("PKCS#11: PQC Kyber Decapsulation Operation");
    ...
    if (ret == 0) {
        ...
        rv = session->func->C_EncapsulateInit(session->handle,
                                              &mech,
                                              publicKey);
        PKCS11_RV("C_EncapsulateInit", rv);
        if (rv != CKR_OK) {
            ret = WC_HW_E;
        }
    }
    ...
    if (ret == 0) {
        rv = session->func->C_Encapsulate(session->handle,
                                (CK_BYTE_PTR)info->pk.pqc_encaps.ciphertext,
                                &ctLen,
                                (CK_BYTE_PTR)info->pk.pqc_encaps.sharedSecret,
                                &ssLen);
        PKCS11_RV("C_Encapsulate", rv);
        if (rv != CKR_OK ||
            (word32)ssLen != info->pk.pqc_encaps.sharedSecretLen ||
...
static int Pkcs11PqcKemDecapsulate(Pkcs11Session* session, wc_CryptoInfo* info)
{
    ...
    if (ret == 0) {
        ...
        rv = session->func->C_DecapsulateInit(session->handle, &mech,
                                              privateKey);
        PKCS11_RV("C_DecapsulateInit", rv);
        if (rv != CKR_OK) {
            ret = WC_HW_E;
        }
    }
                                     ...
                                     CK_MECHANISM_TYPE_PTR hashMech)
{
    ...
    switch (hashType) {
        ...
        case WC_HASH_TYPE_SHAKE128:
            *hashMech = CKM_SHAKE128;
            break;
    #endif
    #ifdef WOLFSSL_SHAKE256
        case WC_HASH_TYPE_SHAKE256:
            *hashMech = CKM_SHAKE256;
            break;
    #endif
        default:
            ret = BAD_FUNC_ARG;
...
static int Pkcs11PqcSigSign(Pkcs11Session* session, wc_CryptoInfo* info)
{
    ...
    if (ret == 0) {
        ...
        if (info->pk.pqc_sign.context != NULL && info->pk.pqc_sign.contextLen > 0) {
            paramSet.pContextData = (byte*) info->pk.pqc_sign.context;
            paramSet.ulContextDataLen = info->pk.pqc_sign.contextLen;

            mech.pParameter = &paramSet;
            mech.ulParameterLen = sizeof(paramSet);
        }
        else {
            paramSet.pContextData = NULL;
            paramSet.ulContextDataLen = 0;
        }

        /* Set preHash data */
        if (info->pk.pqc_sign.preHashType != WC_HASH_TYPE_NONE) {
            paramSet.phFlag = CK_TRUE;
            ...
            paramSet.phFlag = CK_FALSE;
            /* paramSet.hash parameter is ignored */
        }
    }
    if (ret == 0) {
        rv = session->func->C_SignInit(session->handle, &mech, privateKey);
...
static int Pkcs11PqcSigVerify(Pkcs11Session* session, wc_CryptoInfo* info)
{
    ...
    if (ret == 0) {
        ...
        if (info->pk.pqc_verify.context != NULL &&
            ...
            paramSet.pContextData = (byte*) info->pk.pqc_verify.context;
            paramSet.ulContextDataLen = info->pk.pqc_verify.contextLen;
    ...
    const unsigned char* publicKey, word32 pubKeySize)
{
    ...
    /* Compare the size and data of the provided public key with the data
    ...
    if (ret == 0) {
        if (pubKeySize == storedKeySize) {
            ret = XMEMCMP(privateKey->p, publicKey, pubKeySize);
            if (ret != 0)
                ret = MP_CMP_E;
        }
        else
...
static int Pkcs11PqcSigCheckPrivKey(Pkcs11Session* session, wc_CryptoInfo* info)
{
    ...
    if ((key->prvKeySet == 0) || (key->pubKeySet == 0)) {
        /* Get the dilithium private key object. */
        if (key->labelLen > 0) {
